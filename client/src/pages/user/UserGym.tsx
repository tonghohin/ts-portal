...
function Contextmenu(props: Props) {
  const userReducer = useAppSelector((store) => store.user);
  const userGymReducer = useAppSelector((store) => store.userGym);

  async function handleClick(e: React.MouseEvent<HTMLButtonElement>): Promise<void> {
    try {
      await fetch(`/gym/${userGymReducer.id}/${userGymReducer.text}/${userGymReducer.slot}`, { method: "PUT", body: JSON.stringify({ action: e.currentTarget.textContent, unit: userReducer.unit }), headers: { "Content-Type": "application/json" } });
